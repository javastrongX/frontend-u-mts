...
import MobileHotOfferDetails from "./MobileHotOfferDetails";
import SideTranslator from "../SideTranslator";
import Header from "../UserPageLayout/Header";
import { FaDollarSign } from "react-icons/fa";

const MobileHotOfferDetailsLayout = () => {
  ...
  return (
      <>
        <Container bg={{base: bgColor, custom900: "black.0"}} maxW="75rem" display={'flex'} flexDir={'column'} overflow={'hidden'}>
            ...
            <SideTranslator />
            {!isMd && <Header title={t("contactBottomModal.titleAd", "Детали объявления")} subtitle="" MainIcon={FaDollarSign} />}
            <MobileHotOfferDetails />
            <ScrollToTopButton />
            {isMd && <BottomNav />}
            <Box display={isMobile ? 'none' : 'block'}>
              <Footer />
...
export default MobileHotOfferDetailsLayout;
