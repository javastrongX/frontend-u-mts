    ...
    <Box maxW={'75rem'} m={'auto'} h={'100%'} display={'flex'}  bg="black.0" w={'100%'} py={14} minH={'300px'}>
      <HStack gap={20} ml={{base: 10, custom1200: 2}} mr={{base: 10, custom1200: 2}} justify="space-between" align={'flex-start'} flexWrap="nowrap">
        <VStack align="flex-start" spacing={3} maxW="200px">
          <Heading fontWeight="medium" fontSize="28px">
              <HStack as={'a'} href='/' cursor={'pointer'}>
                <Image src="/MlogoLight.png" objectFit={'cover'} alt="UZMAT.uz" boxSize="50px" borderRadius={'md'}/>
          ...
          <Text fontSize={'12px'}>{t('footer.available_mobileApp', "Уже доступно для скачивания")}</Text>
        ...
        <VStack align="flex-start" spacing={2}>
          <Text fontWeight="bold">{t('footer.pages', "Страницы")}</Text>
          <Link fontSize={'14px'} href="/">{t('footer.home', "Главная")}</Link>
          <Link fontSize={'14px'} href="/ads?category_id=0">{t('footer.ad', "Объявления")}</Link>
          <Link fontSize={'14px'} href="/companies">{t('footer.companies', "Компании")}</Link>
          <Link fontSize={'14px'} href="#">{t('footer.support', "Поддержка")}</Link>
        </VStack>

        <VStack align="flex-start" spacing={2}>
          <Text fontWeight="bold">{t('footer.services', "Услуги")}</Text>
          <Link fontSize={'14px'} href="/ads?category_id=0">{t('footer.ads', "Объявления")}</Link>
          <Link fontSize={'14px'} href="/ads?category_id=2">{t('footer.rent', "Аренда")}</Link>
          <Link fontSize={'14px'} href="/applications">{t('footer.orders', "Заказы")}</Link>
        </VStack>

        <VStack align="flex-start" spacing={2}>
          <Text fontWeight="bold">{t('footer.document', "Документы")}</Text>
          <Link fontSize={'14px'} href="#">{t('footer.terms_of_use', "Условия использования")}</Link>
          <Link fontSize={'14px'} href="/ads?category_id=2">{t('footer.rent', "Аренда")}</Link>
          <Link fontSize={'14px'} href="/applications">{t('footer.orders', "Заказы")}</Link>
        </VStack>

        <VStack align="flex-start" spacing={2}>
          <Text fontWeight="bold">{t('footer.contacs', "Контакты")}</Text>
          <Text>+998 (20) 005-9890</Text>
