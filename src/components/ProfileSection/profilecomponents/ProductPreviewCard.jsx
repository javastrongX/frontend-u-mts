...
const ProductPreviewCard = ({ 
  ...
  const [viewsAnimation, setViewsAnimation] = useState(product?.statistics?.viewed || 61);
  ...
  // Animate views for preview mode
  useEffect(() => {
    if (isPreview) {
  ...
  // Rank konfiguratsiyasi
  const getRankConfig = () => {
    if (rank_premium && rank_search && rank_hotOffer) {
      return {
        level: 'premium',
        text: t('AdPreview.premium', "ПРЕМИУМ"),
        icon: FaCrown,
  ...
  // Preview mode uchun qo'shimcha header
    ...
    return (
      <HStack justify="space-between" mb={3}>
        <Text fontSize="sm" color="gray.600" fontWeight="medium">
          {t('AdPreview.previewTitle', "Предосмотр объявления")}
        </Text>
        <HStack spacing={4} fontSize="sm" color="gray.600">
          ...
          >
            ...
            <Text color="blue.600" fontWeight="medium">
              {viewsAnimation} {t('AdPreview.views', "просмотров")}
            </Text>
          </HStack>
        </HStack>
      </HStack>
  ...
  // Stickers ni render qilish
  const renderStickers = () => {
    return (
      <Stack direction="row" wrap="nowrap" spacing={2}>
        {selectedStickers.slice(0, 6).map(stickerId => {
  ...
  return (
    ...
    >
      ...
      <Icon
        as={(product.is_favorite || false) ? FaHeart : FaRegHeart}
        position="absolute"
        top={{ base: 3, sm: 4 }}
        right={{ base: 3, sm: 4 }}
        fontSize={{ base: "18px", sm: "20px" }}
        color={(product.is_favorite || false) ? "red.400" : "gray.400"}
        cursor="pointer"
        onClick={handleLikeClick}
        transition="all 0.2s ease"
        _hover={{
          transform: "scale(1.1)",
          color: (product.is_favorite || false) ? "red.500" : "gray.500"
        }}
      />
      ...
      >
        ...
        <Text
          ...
          flexShrink={0}
        >
          {(!product.prices || product.prices.length === 0 || product.prices[0].type === "negotiable") ? (
      ...
      {/* Category Badge - faqat preview mode da ko'rsatish */}
      {isPreview && product.category && (
        <Badge colorScheme="blue" borderRadius="md" px={2} py={1} mt={2}>
      ...
      >
        ...
        <HStack spacing={{ base: 1, sm: 2 }} flexShrink={0}>
          <HStack spacing={1}>
            ...
            <Text fontSize={{ base: '10px', sm: 'sm' }}>
              {isPreview ? viewsAnimation : (product.statistics?.viewed || 0)}
            </Text>
          </HStack>
