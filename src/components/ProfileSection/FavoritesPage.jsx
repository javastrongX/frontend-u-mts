...
const FavoritesPage = () => {
  ...
  // Helper function to format price based on type
  const formatPrice = (item) => {
    if (!item.prices || item.prices.length === 0) return item.price;
    
    const priceData = item.prices[0];
    if (priceData.type === "negotiable") {
      return "Договорная";
    }
    
    // Format regular price with currency
    const formattedPrice = new Intl.NumberFormat('ru-RU').format(priceData.price);
    const currencyId = priceData.currency;
  ...
  const handleRemoveFromFavorites = async (itemId) => {
    ...
    try {
      setTimeout(() => {
        ...
        toast({
          title: t("favorites.removed", "Удалено из избранного"),
          status: "success",
          duration: 2000,
          isClosable: true,
        });
      ...
      toast({
        title: t("favorites.error", "Ошибка"),
        description: t("favorites.removeFailed", "Не удалось удалить товар"),
        status: "error",
        duration: 3000,
        isClosable: true,
      });
            ...
            <VStack spacing={4} align="stretch" height="100%">
              ...
              <VStack align="stretch" spacing={3} flex="1">
                ...
                <Text
                  fontSize="2xl"
                  fontWeight="800"
                  color={item.prices?.[0]?.type === "negotiable" ? "orange.500" : "blue.500"}
                  bgGradient={item.prices?.[0]?.type === "negotiable" ? "linear(to-r, orange.400, orange.600)" : "linear(to-r, blue.400, blue.600)"}
                  bgClip="text"
                >
                  {formatPrice(item)}
                </Text>
            ...
            <HStack justify="space-between" align="center" display={{base: "none", custom570: "flex"}}>
              <HStack spacing={4}>
                ...
                <Heading size="xl" color={headingColor}>
                  {t("favorites.title", "Избранные товары")}
                </Heading>
              </HStack>
              ...
              {!isLoading && favorites.length > 0 && (
                <Badge
                  ...
                  borderRadius="full"
                >
                    {favorites.length} {t("favorites.count", "товаров")}
                </Badge>
              )}
            </HStack>
                ...
                <VStack spacing={6}>
                  ...
                  <VStack spacing={2}>
                    <Text fontSize="xl" fontWeight="600" color={headingColor}>
                      {t("favorites.empty", "У вас пока нет избранных товаров")}
                    </Text>
                    <Text fontSize="md" color={textColor} textAlign="center">
                      {t("favorites.hint", "Добавляйте интересные товары в избранное, чтобы не потерять их")}
                    </Text>
                  </VStack>
                </VStack>
              </Center>
