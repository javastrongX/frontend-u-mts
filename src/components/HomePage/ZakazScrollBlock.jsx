...
const ZakazScrollBlock = () => {
  ...
  // Block uchun unique ID ishlatish
  const { track } = useStats('zakaz-scroll-block');
  ...
  // To'g'ri navigation handler
  const handleRedirect = (order) => {
    return () => { // Function qaytarish
      track('clicks');
      navigate(`/order-details/product/${order.link}`);
    };
  };
      ...
      >
        {mockOrders.slice(0, 10).map((order) => (
          <Box
            ...
            flexShrink={0}
            cursor="pointer"
            onClick={handleRedirect(order)} // Function call qilish
          >
