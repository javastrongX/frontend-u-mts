...
const HotOfferSection = () => {
  ...
  const currencyId = products?.prices?.[0]?.currency;
  ...
  if (error) {
    return (
      <Box w="100%" px={4} mt={3}>
        <Flex align="center" justify="flex-start" mb={{base: 3, custom900: 5}} gap={4}>
          <Text fontWeight="700" fontSize={{base: "16px", custom900: '22px'}}>
            {t("hot_offers.hot_offer", "Горячее предложения")}
          </Text>
        </Flex>
    ...
    return (
      <Box w="100%" px={4} mt={3}>
        ...
        <Box textAlign="center" py={8}>
          <Text color="gray.500">{t("hot_offers.notfound", "Горячие предложения пока недоступны.")}</Text>
        </Box>
      </Box>
    );
  }
          ...
          >
            {(isMobile ? products : currentItems).map((item, index) => (
              ...
              >
                ...
                >
                  {item.prices?.[0]?.price
                    ? `${item.prices[0].price.toLocaleString()} ${currency.symbol}`
                    : 'Kelishiladi'
                  }
                </Text>
