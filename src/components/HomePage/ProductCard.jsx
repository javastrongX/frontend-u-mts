...
const ProductCard = ({ product }) => {
  ...
  const { stats, isLiked, toggleLike, track } = useStats(product.id);
  ...
  // View ni avtomatik track qilish (faqat bir marta)
  useEffect(() => {
    if (product.id) {
      track('views');
    }
    ...
    >
          ...
          {(!product.prices || product.prices.length === 0 || product.prices[0].type === "negotiable") ? (
            ...
            <>
              <Text
                ...
                flexShrink={0}
              >
                {product.prices[0]?.price.toLocaleString()} {currency.symbol} {!isArenda && ""}
              ...
              {isArenda && (
                <Text
                  ...
                  flexShrink={0}
                >
                  {product.prices[0]?.price.toLocaleString()} {currency.symbol} {t("isArenda.chas", "/ час")}
                </Text>
              )}
            </>
          )}
      ...
      >
        ...
        <HStack spacing={{ base: 1, sm: 2 }} flexShrink={0}>
          {/* View counter - Updated to use stats */}
          <HStack spacing={1}>
            <Icon as={FaEye} fontSize={{ base: "xs", sm: "sm" }} />
            <Text fontSize={{ base: '10px', sm: 'sm' }}>{stats.views}</Text>
          </HStack>
