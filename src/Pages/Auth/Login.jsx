...
import { useAuth } from "./logic/AuthContext"; // AuthContext import qilish
...
const Login = () => {
  ...
  const validateForm = useCallback(() => {
    ...
    if (!emailOrPhone.trim()) {
      newErrors.emailOrPhone = t("auth_login.email_or_phone_err");
    } else if (inputType === "email") {
      // Email validation
      if (!emailRegex.test(emailOrPhone)) {
        newErrors.emailOrPhone = t("auth_login.email_format_error");
      }
    } else if (inputType === "phone") {
      // Phone validation - faqat raqamlarni tekshirish
      if (!phoneRegex.test(emailOrPhone)) {
        newErrors.emailOrPhone = "Telefon raqami 9 ta raqamdan iborat bo'lishi kerak";
      }
    }

    if (!password.trim()) {
      newErrors.password = t("auth_login.password_err");
  ...
  const handleLogin = useCallback(async (e) => {
    ...
    try {
      ...
      toast({
        ...
        description: error.message || "Login jarayonida xatolik yuz berdi",
        status: "error",
        duration: 4000,
        isClosable: true,
      });
