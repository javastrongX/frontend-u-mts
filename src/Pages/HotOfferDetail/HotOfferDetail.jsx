...
const BreadcrumbNav = ({ category, title, t = () => {} }) => {
...
const Statistics = ({ statistics, t }) => {
  if (!statistics) return null;

  const statsData = [
    { icon: FaEye, value: statistics.viewed || 0, label: t("hotOfferDetail.label_view", "просмотры"), color: "gray" },
    { icon: FaHeart, value: statistics.favorite || 0, label: t("hotOfferDetail.label_sevimli", "избранное"), color: "red" },
    { icon: FaShare, value: statistics.share || 0, label: t("hotOfferDetail.label_ulashish", "поделиться"), color: "blue.400" }
  ];
...
const HotOfferDetail = () => {
    ...
    const fetchCardsDetail = async () => {
      try {
        ...
        const transformedData = {
          ...
          author: data.author || { name: t("hotOfferDetail.unknown", "Неизвестно"), avatar: "", is_company: false, is_official: false },
  ...
  const handleLike = async () => {
    try {
      ...
      const response = await fetch(`https://backend-u-mts.onrender.com/api/hot-offers/${slug}/toggle-favorite`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok || !result.success) {
        setIsLiked(!newLikedState);
        throw new Error(result.message || t("hotOfferDetail.update_favourite_err", "Ошибка обновления избранного"));
      }
      ...
      toast({
        title: newLikedState ? t("hotOfferDetail.add_favourite", "Добавлено в избранное") : t("hotOfferDetail.remove_favourite", "Удалено из избранного"),
        status: "success",
        duration: 2000,
        isClosable: true,
      });
  ...
  useEffect(() => {
    if (details && details.id && slug) {
      const trackView = async () => {
        try {
          const response = await fetch(`https://backend-u-mts.onrender.com/api/hot-offers/${slug}/increment-view`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
        ...
        <SlideFade in={true} offsetY="20px">
          ...
          >
            ...
            <GridItem>
              ...
              {/* Description */}
              <Box>
                <Heading size={headingSize} mb={6}>{t("partsmarketplace.company_info.inform", "Описание")}</Heading>
                <Card borderRadius="xl">
                  <CardBody p={{ base: 4, md: 6 }}>
                    <Text 
                      lineHeight="1.8" 
              ...
              {/* Statistics */}
              <Box>
                <Heading size={headingSize} mb={6}>{t("hotOfferDetail.statistika", "Статистика")}</Heading>
                <Card borderRadius="xl">
                  <CardBody p={{ base: 4, md: 6 }}>
                    <Statistics statistics={details.statistics} t={t} />
                  </CardBody>
                </Card>
              </Box>
              {/* Report Button */}
