  ...
  useEffect(() => {
    const fetchNewsDetail = async () => {
      try {
        ...
        setError(err.message || t("SharedNews.errorOccurred", "Произошла ошибка"));
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <Container maxW={{ base: "100%", md: "4xl", lg: "6xl" }} py={{ base: 4, md: 8 }} px={{ base: 4, md: 6 }}>
        <Text color="gray.500" fontSize={{ base: "md", md: "lg" }}>
          {t("SharedNews.newsNotFound", "Новость не найдена")}
        </Text>
      </Container>
    );
  ...
  return (
    <Container mb={'80px'} maxW={{ base: "100%", md: "4xl", lg: "6xl" }} py={{ base: 4, md: 8 }} px={{ base: 4, md: 6 }}>
      <VStack align="stretch" spacing={{ base: 4, md: 6, lg: 8 }}>
        ...
        >
          <Text as={'a'} href={`/`}>{t("side_nav.home", "Главная")}</Text> &nbsp;/&nbsp; <Text as={'a'} href={`/news`}>Новости</Text> &nbsp;/&nbsp;{" "}
        ...
        >
          ...
          <Box alignSelf={{ base: "flex-start", md: "flex-start" }}>
            <HStack>
              <IconButton
                aria-label={t("SharedNews.share", "Поделиться")}
                icon={<FaShareAlt />}
                colorScheme="yellow"
                variant="solid"
                size={{ base: "sm", md: "md" }}
        ...
        >
          <HStack spacing={{ base: 3, md: 4 }}>
            ...
            <VStack align="flex-start" spacing={0}>
              ...
              >
                {t("SharedNews.newsAuthor", "Автор новости")}
              </Text>
            </VStack>
          </HStack>
          ...
          <Box display={{ base: "none", lg: "block" }}>
            <HStack spacing={2}>
              <Text fontSize="sm" color="gray.500">
                {t("SharedNews.articleLiked", "Понравилась статья?")}
              </Text>
              <IconButton
                aria-label={t("SharedNews.share", "Поделиться снова")}
                icon={<FaShareAlt />}
                size="sm"
                variant="ghost"
                colorScheme="gray"
